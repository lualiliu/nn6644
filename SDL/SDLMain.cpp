...
#define load_sym(V, S) do {\
    if (!((*(void**)&V) = SDL_LoadFunction(handle, #S))) \
        die("Failed to load symbol '" #S "'': %s", SDL_GetError()); \
	} while (0)
#define load_retro_sym(S) load_sym(S, S)
*/
...
//    die("Framebuffer incomplete: 0x%X", status);
//}
...
static void video_configure(const struct retro_game_geometry *geom) {
    ...
    if (is_gles) {
        ...
        printf("Using OpenGL ES compatible format: RGBA/UNSIGNED_BYTE\n");
    } else {
        // 桌面 OpenGL 格式
        internalFormat = GL_RGBA8;
        format = GL_RGBA;
        type = GL_UNSIGNED_BYTE;
        printf("Using Desktop OpenGL format: RGBA8/RGBA/UNSIGNED_BYTE\n");
    }
    
    // 分配纹理存储
    printf("Allocating texture storage: %dx%d, internal=0x%X, format=0x%X, type=0x%X\n",
           geom->max_width, geom->max_height, internalFormat, format, type);
    ...
    if (error != GL_NO_ERROR) {
        printf("glTexImage2D failed with error: 0x%X\n", error);
        
        // 尝试回退方案
        printf("Trying fallback formats...\n");
        ...
        error = glGetError();
        if (error == GL_NO_ERROR) {
            printf("Fallback 1 (RGBA) succeeded\n");
        } else {
            printf("Fallback 1 failed: 0x%X\n", error);
            ...
            error = glGetError();
            if (error == GL_NO_ERROR) {
                printf("Fallback 2 (RGB) succeeded\n");
            } else {
                printf("Fallback 2 failed: 0x%X\n", error);
                die("All texture format attempts failed");
            }
        }
    }
    ...
    printf("Texture verification: %dx%d (requested: %dx%d)\n", 
           texWidth, texHeight, geom->max_width, geom->max_height);
    
    if (texWidth == 0 || texHeight == 0) {
        die("Texture storage allocation failed - zero dimensions");
    }
    
    if (texWidth != geom->max_width || texHeight != geom->max_height) {
        printf("Warning: Texture size mismatch, but continuing...\n");
    }
...
static void core_load_game(const char *filename) {
    ...
    snprintf(window_title, sizeof(window_title), "nn6644", system.library_name, system.library_version);
...
*/
    ...
    if (!supports_no_game && argc < 2)
        die("This core requires a game in order to run");

    // Load the game.
    core_load_game(argc > 1 ? argv[1] : NULL);
